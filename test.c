#include <aes.h>

#include <stdio.h>
#include <string.h>

#define STR_PASSED "\033[32mPASSED\033[0m!\n"
#define STR_FAILED "\033[31mFAILED\033[0m!\n"

int test_rot_word() {
	printf("Testing bb_rot_word()...");
	uint8_t expected[] = {0x01, 0x02, 0x03, 0x00};
	uint8_t initial[] = {0x00, 0x01, 0x02, 0x03};
	bb_rot_word(initial, initial);
	if (memcmp(expected, initial, sizeof(expected)) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_sbox_word() {
	printf("Testing bb_sbox_word()...");
	uint8_t expected[] = {0x7c, 0x25, 0x0b, 0x77};
	uint8_t initial[] = {0x01, 0xc2, 0x9e, 0x02};
	bb_sbox_word(initial, initial);
	if (memcmp(expected, initial, sizeof(expected)) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_rcon_word() {
	printf("Testing bb_rcon_word()...");
	uint8_t expected[] = {0x08, 0x00, 0x00, 0x00};
	uint8_t initial[] = {0x00, 0x00, 0x00, 0x00};
	bb_rcon_word(4, initial);
	if (memcmp(expected, initial, sizeof(expected)) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_key_expansion_ex(uint8_t* key, uint8_t key_size, uint8_t* expected, int expected_size) {
	int key_num = 0;
	for (int bytes_expanded = 0; bytes_expanded < expected_size; bytes_expanded += key_size) {
		bb_key_expansion(key, key_size, key, key_num + 1);
		int bytes_remaining = expected_size - bytes_expanded;
		int comparison_size = bytes_remaining > key_size ? key_size : bytes_remaining;
		if (memcmp(key, expected + (key_num * key_size), comparison_size) != 0) {
			printf(STR_FAILED);
			return 1;
		}
		key_num += 1;
	}
	printf(STR_PASSED);
	return 0;
}

int test_key_expansion_128() {
	printf("Testing bb_key_expansion() w/ 128 byte key...");
	uint8_t key[] = {
		0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
		0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
	};

	uint8_t expected[] = {
		
	};
	return test_key_expansion_ex(key, sizeof(key), expected, sizeof(expected));
}


int test_key_expansion_192() {
	printf("Testing bb_key_expansion() w/ 192 byte key...");
	uint8_t key[] = {
		0x8e, 0x73, 0xb0, 0xf7, 0xda, 0x0e, 0x64, 0x52,
		0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5,
		0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b
	};
	uint8_t expected[] = {
		,
	};
	return test_key_expansion_ex(key, sizeof(key), expected, sizeof(expected));
}

int test_key_expansion_256() {
	printf("Testing bb_key_expansion() w/ 256 bytes key...");
	uint8_t key[] = {
		0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
		0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
		0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7,
		0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4,
	};
	uint8_t expected[] = {
		
	};
	return test_key_expansion_ex(key, sizeof(key), expected, sizeof(expected));
}

int test_sbox_state() {
	printf("Testing bb_sbox_state()...");
	uint8_t state[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
	};
	uint8_t expected[] = {
		0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
		0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76
	};
	bb_sbox_state(state, state);
	
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_inv_sbox_state() {
	printf("Testing bb_inv_sbox_state()...");
	uint8_t state[] = {
		0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
		0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76
	};
	uint8_t expected[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
	};
	
	bb_inv_sbox_state(state, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_shift_state() {
	printf("Testing bb_shift_state()...");
	uint8_t state[] = {
		0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
		0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76
	};
	uint8_t expected[] = {
		0x63, 0x6b, 0x67, 0x76, 0xf2, 0x01, 0xab, 0x7b,
		0x30, 0xd7, 0x77, 0xc5, 0xfe, 0x7c, 0x6f, 0x2b
	};
	bb_shift_state(state, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_inv_shift_state() {
	printf("Testing bb_inv_shift_state()...");
	uint8_t state[] = {
		0x63, 0x6b, 0x67, 0x76, 0xf2, 0x01, 0xab, 0x7b,
		0x30, 0xd7, 0x77, 0xc5, 0xfe, 0x7c, 0x6f, 0x2b
	};
	uint8_t expected[] = {
		0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
		0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76
	};
	bb_inv_shift_state(state, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_mix_columns() {
	printf("Testing bb_mix_columns_enc()...");
	uint8_t state[] = {
		0x63, 0x6b, 0x67, 0x76, 0xf2, 0x01, 0xab, 0x7b,
		0x30, 0xd7, 0x77, 0xc5, 0xfe, 0x7c, 0x6f, 0x2b
	};
	uint8_t expected[] = {
		0x6a, 0x6a, 0x5c, 0x45, 0x2c, 0x6d, 0x33, 0x51,
		0xb0, 0xd9, 0x5d, 0x61, 0x27, 0x9c, 0x21, 0x5c
	};
	bb_mix_columns(state, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_inv_mix_columns() {
	printf("Testing bb_mix_columns_dec()...");
	uint8_t state[] = {
		0x6a, 0x6a, 0x5c, 0x45, 0x2c, 0x6d, 0x33, 0x51,
		0xb0, 0xd9, 0x5d, 0x61, 0x27, 0x9c, 0x21, 0x5c
	};
	uint8_t expected[] = {
		0x63, 0x6b, 0x67, 0x76, 0xf2, 0x01, 0xab, 0x7b,
		0x30, 0xd7, 0x77, 0xc5, 0xfe, 0x7c, 0x6f, 0x2b
	};
	bb_inv_mix_columns(state, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_add_round_key() {
	printf("Testing bb_add_round_key()...");
	uint8_t state[] = {
		0x6a, 0x6a, 0x5c, 0x45, 0x2c, 0x6d, 0x33, 0x51,
		0xb0, 0xd9, 0x5d, 0x61, 0x27, 0x9c, 0x21, 0x5c
	};
	uint8_t round_key[] = {
		0xd6, 0xaa, 0x74, 0xfd, 0xd2, 0xaf, 0x72, 0xfa,
		0xda, 0xa6, 0x78, 0xf1, 0xd6, 0xab, 0x76, 0xfe
	};
	uint8_t expected[] = {
		0xbc, 0xc0, 0x28, 0xb8, 0xfe, 0xc2, 0x41, 0xab,
		0x6a, 0x7f, 0x25, 0x90, 0xf1, 0x37, 0x57, 0xa2
	};
	bb_add_round_key(state, round_key, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
};

int test_full_round() {
	printf("Testing full round...");
	uint8_t state[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
	};
	uint8_t round_key[] = {
		0xd6, 0xaa, 0x74, 0xfd, 0xd2, 0xaf, 0x72, 0xfa,
		0xda, 0xa6, 0x78, 0xf1, 0xd6, 0xab, 0x76, 0xfe
	};
	uint8_t expected[] = {
		0xbc, 0xc0, 0x28, 0xb8, 0xfe, 0xc2, 0x41, 0xab,
		0x6a, 0x7f, 0x25, 0x90, 0xf1, 0x37, 0x57, 0xa2
	};
	bb_sbox_state(state, state);
	bb_shift_state(state, state);
	bb_mix_columns(state, state);
	bb_add_round_key(state, round_key, state);
	if (memcmp(state, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_encrypt() {
	printf("Testing bb_encrypt_128()...");
	uint8_t plaintext[] = {'t','h','e','b','l','o','c','k','b','r','e','a','k','e','r','s'};
	uint8_t key[] = {
		0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
		0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
	};
	uint8_t ciphertext[AES_128_KEY_BYTES] = {};
	uint8_t expected[] = {
		0xc6, 0x9f, 0x25, 0xd0, 0x02, 0x5a, 0x9e, 0xf3,
		0x23, 0x93, 0xf6, 0x3e, 0x2f, 0x05, 0xb7, 0x47
	};
	bb_encrypt_128(plaintext, key, ciphertext);
	if (memcmp(ciphertext, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_encrypt_128() {
	printf("Testing bb_encrypt_128() with NIST vector...");
	uint8_t plaintext[] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};
	uint8_t key[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
	};
	uint8_t ciphertext[AES_128_KEY_BYTES] = {};
	uint8_t expected[] = {
		0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30,
		0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a
	};
	bb_encrypt_128(plaintext, key, ciphertext);
	if (memcmp(ciphertext, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_encrypt_192() {
	printf("Testing bb_encrypt_192() with NIST vector...");
	uint8_t plaintext[] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};
	uint8_t key[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
		0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17
	};
	uint8_t ciphertext[sizeof(plaintext)] = {};
	uint8_t expected[sizeof(plaintext)] = {
		0xdd, 0xa9, 0x7c, 0xa4, 0x86, 0x4c, 0xdf, 0xe0,
		0x6e, 0xaf, 0x70, 0xa0, 0xec, 0x0d, 0x71, 0x91
	};

	bb_encrypt_192(plaintext, key, ciphertext);
	if (memcmp(ciphertext, expected, sizeof(ciphertext)) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_encrypt_256() {
	printf("Testing bb_encrypt_256() with NIST vector...");
	uint8_t plaintext[] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};
	uint8_t key[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
		0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
		0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f
	};
	uint8_t ciphertext[AES_BLOCK_SIZE] = {};
	uint8_t expected[] = {
		0x8e, 0xa2, 0xb7, 0xca, 0x51, 0x67, 0x45, 0xbf,
		0xea, 0xfc, 0x49, 0x90, 0x4b, 0x49, 0x60, 0x89
	};
	bb_encrypt_256(plaintext, key, ciphertext);
	if (memcmp(ciphertext, expected, AES_BLOCK_SIZE) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_decrypt() {
	printf("Testing bb_decrypt_128()...");
	uint8_t ciphertext[] = {
		0xc6, 0x9f, 0x25, 0xd0, 0x02, 0x5a, 0x9e, 0xf3,
		0x23, 0x93, 0xf6, 0x3e, 0x2f, 0x05, 0xb7, 0x47
	};
	uint8_t key[] = {
		0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
		0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
	};
	uint8_t plaintext[AES_128_KEY_BYTES] = {};
	uint8_t expected[] = {'t','h','e','b','l','o','c','k','b','r','e','a','k','e','r','s'};
	
	bb_decrypt_128(ciphertext, key, plaintext);
	if (memcmp(plaintext, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_decrypt_128() {
	printf("Testing bb_decrypt_128() with NIST vector...");
	uint8_t ciphertext[] = {
		0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30,
		0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a
	};
	uint8_t key[] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f
	};
	uint8_t plaintext[AES_128_KEY_BYTES] = {};
	uint8_t expected[] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};
	bb_decrypt_128(ciphertext, key, plaintext);
	if (memcmp(plaintext, expected, AES_128_KEY_BYTES) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_decrypt_192() {
	printf("Testing bb_decrypt_192() with NIST vector...");
	uint8_t ciphertext[AES_BLOCK_SIZE] = {
		0xdd, 0xa9, 0x7c, 0xa4, 0x86, 0x4c, 0xdf, 0xe0,
		0x6e, 0xaf, 0x70, 0xa0, 0xec, 0x0d, 0x71, 0x91
	};
	uint8_t key[AES_192_KEY_BYTES] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
		0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17
	};
	uint8_t plaintext[AES_BLOCK_SIZE] = {};
	uint8_t expected[] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};

	bb_decrypt_192(ciphertext, key, plaintext);
	if (memcmp(plaintext, expected, AES_BLOCK_SIZE) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int test_bb_decrypt_256() {
	printf("Testing bb_decrypt_256() with NIST vector...");
	uint8_t ciphertext[AES_BLOCK_SIZE] = {
		0x8e, 0xa2, 0xb7, 0xca, 0x51, 0x67, 0x45, 0xbf,
		0xea, 0xfc, 0x49, 0x90, 0x4b, 0x49, 0x60, 0x89
	};
	uint8_t key[AES_256_KEY_BYTES] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
		0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
		0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
		0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f
	};
	uint8_t plaintext[AES_BLOCK_SIZE] = {};
	uint8_t expected[AES_BLOCK_SIZE] = {
		0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
		0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff
	};

	bb_decrypt_256(ciphertext, key, plaintext);
	if (memcmp(plaintext, expected, sizeof(expected)) != 0) {
		printf(STR_FAILED);
		return 1;
	} else {
		printf(STR_PASSED);
		return 0;
	}
}

int main() {
	int failures = 0;
	failures += test_rot_word();
	failures += test_sbox_word();
	failures += test_rcon_word();
	failures += test_key_expansion_128();
	failures += test_key_expansion_192();
	failures += test_key_expansion_256();
	failures += test_sbox_state();
	failures += test_inv_sbox_state();
	failures += test_shift_state();
	failures += test_inv_shift_state();
	failures += test_mix_columns();
	failures += test_inv_mix_columns();
	failures += test_add_round_key();
	failures += test_full_round();
	failures += test_encrypt();
	failures += test_bb_encrypt_128();
	failures += test_bb_encrypt_192();
	failures += test_bb_encrypt_256();
	failures += test_decrypt();
	failures += test_bb_decrypt_128();
	failures += test_bb_decrypt_192();
	failures += test_bb_decrypt_256();
	if (failures == 0) {
		printf("All tests " STR_PASSED);
	} else if (failures == 1) {
		printf("%d test " STR_FAILED, failures);
	} else {
		printf("%d tests " STR_FAILED, failures);
	}
	
	return failures;
}

